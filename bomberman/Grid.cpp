...
void Grid::draw(Square square, MySDL::Texture &texture, double angle) {
	Rectangle const cell = toGridCell(square);
	::draw(renderer_, texture, cell, angle);
}

void Grid::fill(Square square, Rgb rgb) {
...
Rectangle Grid::toGridCell(Square const &square) {
...
SDL_Rect Grid::toPhysical(Square const &square) {
	Rectangle const cell = toGridCell(square);
	return toSDL(cell);
}
