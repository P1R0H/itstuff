...
public:
	 ...
	 */
	void fill(Square square, Rgb rgb);

	/**
	 * @brief Draws 'texture' rotated by 'angle' to 'square'.
	 */
	void draw(Square square, MySDL::Texture &texture, double angle);

private:
	Vector physicalSize() const;
	Rectangle toGridCell(Square const &square);
	SDL_Rect toPhysical(Square const &square);
	void drawInnerGrid();
	void drawBorder();
