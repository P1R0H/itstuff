...
private:
	...
	bool stop = false;
	chrono::steady_clock::time_point next_game_tick{};
...
void App::main() {
	const auto gametick_dt = Config::cellTime / Config::mesh;
	const auto render_dt = 16ms;
	next_game_tick = next_render = chrono::steady_clock::now();

	while (!stop) {
		auto currentTime = chrono::steady_clock::now();

		if (currentTime >= next_game_tick) {
			next_game_tick += gametick_dt;
			gameTick();
		}
		...
		std::this_thread::sleep_until(std::min(next_render, next_game_tick));
	}
}
...
/**
 * @brief Represents logical step in the game.
 */
void App::gameTick() {
...
void App::renderMapItems() {
	...
	grid.draw(bomberman.dimensions.coarseSquare(center.x, center.y), textures.get({ "human.png" }), 0);
#endif
}

struct Options {
	bool help = false;
	string gameMapFile{ "maps/classic.txt" };
...
Options parseOptions(int argc, char *argv[]) {
	...
	if (string(argv[1]) == "--help") {
		cout << "Usage: " << argv[0] << " [name_of_map.txt]" << endl;
		exit(0);
	}

	options.gameMapFile = argv[1];
